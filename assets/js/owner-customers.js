...
(function () {
  ...
  function statusBadge(status) {
    ...
    var colors = { 'Active': '#00b140', 'active': '#00b140', 'Inactive': '#cc0000', 'inactive': '#cc0000', 'Pending': '#ca8a04', 'pending': '#ca8a04', 'Suspended': '#cc0000', 'suspended': '#cc0000' };
  ...
  function subBadge(status) {
    if (!status) return '<span style="color:#999;">—</span>';
    var colors = { 'paid': '#00b140', 'free': '#2563eb', 'trial': '#ca8a04', 'expired': '#cc0000' };
    var color = colors[status] || '#666';
    return '<span style="font-size:12px;font-weight:600;color:' + color + ';padding:2px 8px;border-radius:4px;background:' + color + '15;">' + status + '</span>';
  }
  ...
  async function loadCustomers() {
    ...
    try {
      ...
      if (!data.success) { container.innerHTML = '<div style="text-align:center;padding:40px;color:#cc0000;">' + (data.error || 'Failed to load customers') + '</div>'; return; }
      allCustomers = data.customers || [];
      filteredCustomers = allCustomers.slice();
      renderCustomers();
    } catch (err) {
      container.innerHTML = '<div style="text-align:center;padding:40px;color:#cc0000;">Failed to load customers. The DRIV-EN team has been notified.</div>';
      if (window.DRIVENErrorHandler) DRIVENErrorHandler.report('Failed to load customers', 'owner-customers', err.stack);
    }
  }
  ...
  function renderCustomers() {
    var container = document.getElementById('customers-content');
    if (!container) return;
    if (filteredCustomers.length === 0) { container.innerHTML = renderToolbar() + '<div style="text-align:center;padding:40px;color:#999;">No customers found.</div>'; return; }
  ...
  function renderToolbar() {
    return '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:12px;">' +
      ...
      '<div style="display:flex;gap:8px;align-items:center;">' +
        '<input type="text" id="customer-search" placeholder="Search by company, email, city..." value="' + searchQuery + '" oninput="window.OWNER_CUSTOMERS.search(this.value)" style="padding:8px 12px;border:1px solid #ddd;border-radius:6px;font-size:13px;width:260px;" />' +
        '<button onclick="window.OWNER_CUSTOMERS.refresh()" style="padding:8px 16px;background:#00b140;color:#fff;border:none;border-radius:6px;font-size:13px;cursor:pointer;">Refresh</button>' +
      '</div></div>';
  }
  ...
  function search(query) {
    searchQuery = query;
    if (!query || query.trim() === '') { filteredCustomers = allCustomers.slice(); }
    else {
      var q = query.toLowerCase().trim();
      filteredCustomers = allCustomers.filter(function (c) {
        return (c.company_name && c.company_name.toLowerCase().includes(q)) || (c.admin_email && c.admin_email.toLowerCase().includes(q)) || (c.city && c.city.toLowerCase().includes(q)) || (c.state && c.state.toLowerCase().includes(q)) || (c.customer_id && c.customer_id.toLowerCase().includes(q)) || (c.activation_code && c.activation_code.toLowerCase().includes(q));
      });
    }
    renderCustomers();
  ...
  function refresh() { loadCustomers(); }

  window.OWNER_CUSTOMERS = { load: loadCustomers, toggleRow: toggleRow, search: search, refresh: refresh };

  function checkAndLoad() {
    var container = document.getElementById('customers-content');
    if (!container) return;
    var customersTabBtn = document.querySelector('.tab-btn[data-tab="customers"]');
    if (customersTabBtn) { customersTabBtn.addEventListener('click', function () { if (allCustomers.length === 0) loadCustomers(); }); }
    var customersPanel = document.getElementById('tab-customers');
    if (customersPanel && customersPanel.style.display !== 'none') { loadCustomers(); }
  }

  if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', checkAndLoad); }
  else { checkAndLoad(); }
})();
