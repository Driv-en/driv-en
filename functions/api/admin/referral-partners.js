...
//
// LAST UPDATED: September 5, 2026 (Session 19)
// CHANGES:
//   - Added payment_info LEFT JOIN to GET query (ACH status per partner)
//   - Auto-creates payment_info table before querying
//   - Previous: last_referred subquery, toggle_active, reapprove_w9, logo emails
// ============================================================================
...
// ---------------------------------------------------------------------------
// GET — List all partners (with payment_info LEFT JOIN)
// ---------------------------------------------------------------------------
async function handleListPartners(request, env) {
  const user = await verifyFounder(request, env);
  if (!user) return jsonResponse({ success: false, error: 'Unauthorized — DRIV-EN Founder access required' }, 403);

  try {
    // Ensure payment_info table exists before querying
    try {
    ...
    const result = await env.DB.prepare(
      `SELECT rp.id, rp.partner_name, rp.partner_email, rp.partner_phone, rp.status,
              ...
              pi.bank_name, pi.account_type, pi.account_number_last4,
              pi.name_on_account, pi.voided_check_attachment as has_voided_check
       FROM referral_partners rp
       LEFT JOIN payment_info pi ON pi.partner_id = rp.id
       ORDER BY
    ...
    return jsonResponse({ success: true, partners: result.results || [] });
  } catch (e) {
...
// ---------------------------------------------------------------------------
// POST — Approve, reject, toggle_active, or reapprove_w9
// ---------------------------------------------------------------------------
async function handleUpdatePartner(request, env) {
  ...
  if (!action || !partnerId) return jsonResponse({ success: false, error: 'Action and partnerId are required' }, 400);

  const partner = await env.DB.prepare(
    'SELECT id, partner_name, partner_email, status FROM referral_partners WHERE id = ?'
  ).bind(partnerId).first();
  if (!partner) return jsonResponse({ success: false, error: 'Partner not found' }, 404);

  if (action === 'approve') {
    if (!referralCode) return jsonResponse({ success: false, error: 'Referral code is required' }, 400);
    ...
    if (existingCode) return jsonResponse({ success: false, error: 'Referral code already in use by another partner' }, 409);
    ...
    return jsonResponse({
      ...
      referralCode, w9ExpirationDate: w9Exp, tempPassword,
      emailSent: emailResult.ok, emailError: emailResult.ok ? null : emailResult.error
    });

  } else if (action === 'reject') {
    await env.DB.prepare(
      `UPDATE referral_partners SET status = 'Rejected', active = 0, is_eligible = 0, partner_status = 'Rejected' WHERE id = ?`
    ).bind(partnerId).run();
    ...
    return jsonResponse({ success: true, message: 'Partner rejected. Notification email sent to ' + partner.partner_email });

  } else if (action === 'reapprove_w9') {
    const w9Partner = await env.DB.prepare('SELECT status, referral_code FROM referral_partners WHERE id = ?').bind(partnerId).first();
    if (!w9Partner) return jsonResponse({ success: false, error: 'Partner not found' }, 404);
    if (w9Partner.status !== 'Pending W-9 Review') return jsonResponse({ success: false, error: 'Can only re-approve partners with W-9 pending review' }, 400);
    await env.DB.prepare(`UPDATE referral_partners SET status = 'Approved', active = 1, is_eligible = 1, partner_status = 'Approved' WHERE id = ?`).bind(partnerId).run();
    return jsonResponse({ success: true, message: 'Partner re-approved. Referral link reactivated.' });

  } else if (action === 'toggle_active') {
    const currentPartner = await env.DB.prepare('SELECT active, status FROM referral_partners WHERE id = ?').bind(partnerId).first();
    if (!currentPartner) return jsonResponse({ success: false, error: 'Partner not found' }, 404);
    if (currentPartner.status !== 'Approved') return jsonResponse({ success: false, error: 'Can only toggle active state for Approved partners' }, 400);
    const newActive = currentPartner.active === 1 ? 0 : 1;
    await env.DB.prepare('UPDATE referral_partners SET active = ? WHERE id = ?').bind(newActive, partnerId).run();
    return jsonResponse({ success: true, message: newActive === 1 ? 'Partner activated' : 'Partner deactivated', active: newActive });
